...
from colorization_model import colorize_image, enhance_image, detect_faces, colorize_with_advanced_model
import time
...
@st.cache_data
def process_image(_image_array, process_option):
    """
    Process the image based on selected mode with caching for better performance.
    Takes a numpy array as input for consistent hashing.
    """
    try:
        ...
        elif process_option == "Detect Faces":
            result_array, num_faces = detect_faces(image)
            return {
                'image': Image.fromarray(result_array.astype('uint8')),
                'caption': f"Detected {num_faces} face{'s' if num_faces != 1 else ''}",
                'filename': "detected_faces.png",
...
def main():
    ...
    with st.sidebar:
        ...
        - **Detect Faces**: Identifies and marks faces in the image
        ...
        try:
            ...
            with col2:
                ...
                if st.download_button(
                    ...
                    st.markdown(f'<div class="success-message">✅ Image downloaded successfully!</div>', 
                              unsafe_allow_html=True)
                
                # Display additional information for face detection
                if process_option == "Detect Faces" and 'metadata' in result:
                    st.info(f"🔍 Number of faces detected: {result['metadata']['num_faces']}")

        except ValueError as ve:
            st.error(f"⚠️ Validation Error: {str(ve)}")
        except RuntimeError as re:
            st.error(f"⚠️ Processing Error: {str(re)}")
        except Exception as e:
            st.error(f"⚠️ An unexpected error occurred: {str(e)}")
            st.error("Please try again with a different image or processing mode.")
    else:
        # Show example images and instructions when no file is uploaded
        with col2:
            st.info("ℹ️ Waiting for image upload...")
            st.markdown("""
            #### Tips:
